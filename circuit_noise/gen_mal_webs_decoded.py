import os
"""Decoded-convention 'why a segment is violet' panels, one per fate:
(a) detected round-2 fault (unchanged); (b) genuinely malignant fault
under the decoded convention: the round-1 Z on the injected qubit at
its Z-check tap (verified in the decoded 13-class list, step 1/6b);
(c) the round-2 Z fault that the standalone view would flag but the
web closing through the NEXT round catches: ordinary decoding sees
it. Output: version_5_v2."""
import sys, os
...
# round-2 CNOT fault locations (for panels a and c)
r2 = []
...
# ---- (a) detected: unchanged from the original panels
det = None
for v, t, ea, et in r2:
    for P in 'XZY':
...
annotate(pa, ea, P)
print(f'(a) detected: {P} on {ea}, caught by closed web {wi}')

# ---- (b) DECODED-malignant: round-1 Z on the injected qubit's wire
# at its Z-check tap (Z(0.5,1.5) -> data (0,2), class Pt=Z), from the
# verified decoded 13-class list.
tap = None
for v in g.vertices():
    rv = meta[v]['role']
    if rv.startswith('atgt') and rv.endswith('r'):
        cell = meta[v].get('cell')
        if cell and cell[1] == 'Z' and tuple(cell[2]) == (0.5, 1.5):
            dp = [w for w in g.neighbors(v)
                  if meta[w]['role'].startswith('dtap')
                  and tuple(map(float, meta[w]['cell'])) == (0.0, 2.0)]
            if dp:
                tap = dp[0]
                break
assert tap is not None, 'round-1 Z-check tap on injected qubit not found'
e_b = out_edge(tap)
P_b = 'Z'
syn, log = classify(e_b, P_b)
assert not any(syn) and log, (syn, log)
# nearest X-decorated closed detector web, which stops short
def mid(e):
    pts = []
    for v in e:
...
for k, vec in enumerate(vecs):
    ...
    if best is None or dmin < best[0]:
        best = (dmin, k, web)
dmin, kb, web_b = best
assert overlap(vecs[kb], e_b, P_b) == 0
pb = os.path.join(OUT, 'fig_malweb_malignant.tex')
emit(g, [dict(web_b)], pb, zscale=2.2, node_size='0.4cm', planes=planes)
annotate(pb, e_b, P_b)
print(f'(b) decoded-malignant: {P_b} on {e_b} (round-1 Z-check tap of '
      f'the injected qubit); nearest X-decorated closed web #{kb} at '
      f'distance {dmin:.2f}, overlap 0')

# ---- (c) round-2 fault caught by the next round (old b+c content)
mal = None
...
annotate(pc, e_c, P_c)
print(f'(c) caught by next round: {P_c} on {e_c}; catching web has '
      f'{len(web_c)} decorated edges, reaches the open outputs')
