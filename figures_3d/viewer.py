...
JS_CANDIDATES = [
    ...
    os.path.join(os.path.dirname(os.path.abspath(__file__)),
                 '..', 'vendor', 'pyzx_3d_viewer', 'zx_viewer_3D.js'),
]
...
# pi/2 spiders (the |Y> inputs and Euler-H spiders) render pale blue for visibility.
# This patches OUR generated page only -- the vendored viewer repo file is untouched.
_PI2_TGT = 'const mesh = new THREE.Mesh(geom, nodeMaterials[n.t] || nodeMaterials[1]);'
_PI2_NEW = ("const mesh = new THREE.Mesh(geom, "
            "((n.phase === '1/2' || n.phase === '3/2') ? "
            "new THREE.MeshBasicMaterial({ color: new THREE.Color('rgb(173,205,240)') }) : "
            "(nodeMaterials[n.t] || nodeMaterials[1])));")
...
def _js_block(js):
    ...
    lib = _js()
    assert _PI2_TGT in lib, 'viewer JS changed: pi/2-blue patch target missing'
    return lib.replace(_PI2_TGT, _PI2_NEW)
...
def compact_z(g, meta=None):
    """Squish the time axis WITHOUT tearing rounds apart.

    Levels are assigned to TICKS (original z values), not vertices: every spider of
    one measurement round moves together, so rounds stay flat. Tick dependencies come
    from cell worldlines (a cell appearing in ticks t1 < t2 orders them); independent
    ticks (disjoint regions) collapse onto shared levels via longest path. Source
    ticks (inits, open inputs) are then placed ALAP: one level below their first
    consumer, so a seam initialised for the third merge window sits AT that window."""
    # cell worldline histories
    hist = {}
    for v in g.vertices():
        z = g.vdata(v, 'z', 0.0)
        cell = None
        if meta is not None and v in meta:
            cell = meta[v].get('cell')
        if cell is None:
            # fall back: group by (row, qubit) spatial position for wire continuity
            cell = ('pos', round(g.row(v), 3), round(g.qubit(v), 3))
        hist.setdefault(cell, set()).add(z)
    ticks = sorted({g.vdata(v, 'z', 0.0) for v in g.vertices()})
    succ = {t: set() for t in ticks}
    pred = {t: set() for t in ticks}
    for cell, zs in hist.items():
        seq = sorted(zs)
        for a, b in zip(seq, seq[1:]):
            succ[a].add(b)
            pred[b].add(a)
    level = {}
    for t in ticks:                                  # ASAP over the tick DAG
        level[t] = max((level[p] + 1 for p in pred[t]), default=0)
    for t in reversed(ticks):                        # ALAP for source ticks
        if not pred[t] and succ[t]:
            level[t] = min(level[u] for u in succ[t]) - 1
    for v in g.vertices():
        g.set_vdata(v, 'z', float(level[g.vdata(v, 'z', 0.0)]))
    return g
